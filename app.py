...
with st.expander("**LIME explanation**"):
    st.write('''
        ...
        It is a procedure that uses local models to explain the predictions made by any model. \n
...
st.markdown("<h2 style='text-align: center; color: Black;'>SHAP Summary Plot</h2>", unsafe_allow_html=True)

col0, col1, col2 = st.columns([2,4,4])  
...
hist_col = col0.selectbox('Select Component to view Query Value and Database Distribution', options = X_train.columns, index=default_ix)
#col2.write('**QUERY VALUE: %s**'%(list(X_test[hist_col])[0]))
col1.markdown("<h6 style='text-align: left; color: orange;'>%s</h6>"%('''Query Value for '%s': %s'''%(hist_col, list(X_test[hist_col])[0])), unsafe_allow_html=True)
plt.subplot(2,3,1)

X_train.hist(hist_col,color = 'grey')
plt.title('''Database Distribution for ' '''[:-1]+ hist_col +"'")
col1.pyplot()
